...
def main(args):
    ...
    from data_processing.DataLoader import FoldersDistributedDataset, \
        get_transform, get_data_loader
    from attn_gan_pytorch.Losses import HingeGAN

    # create a data source:
    dataset = FoldersDistributedDataset(args.images_dir,
                                        transform=get_transform((128, 128)))
    ...
    gen_optim = th.optim.Adam(generator.parameters(), args.g_lr, [0, 0.9])

    dis_optim = th.optim.Adam(discriminator.parameters(), args.d_lr, [0, 0.9])
